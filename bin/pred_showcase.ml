...
let example_1 = V.(
  ...
  let p_imp_p = op(Implies(x_eq_x, x_eq_x)) in
...
let example_2 = V.(P.(
  let subset a b = forall "x" (op(Implies(member "x" a, member "x" b))) in
  let a_ss_b_ss_c = op(And(subset "a" "b", subset "b" "c")) in
  let claim = forall "a" (forall "b" (forall "c" (op(Implies(a_ss_b_ss_c, subset "a" "c"))))) in

  let conj_1 = inference [Op(And(MetaVar 1, MetaVar 2))] (MetaVar 1) in
  let conj_2 = inference [Op(And(MetaVar 1, MetaVar 2))] (MetaVar 2) in
  let m_p = inference [Op(Implies(MetaVar 1, MetaVar 2)); MetaVar 1] (MetaVar 2) in
...
let example_3 = V.(P.(
  let tautology = op(Or(member "x" "y", op(Not(member "x" "y")))) in
  let claim = forall "x" (forall "y" tautology) in

  let p_or_not_p = inference [] (Op(Or(MetaVar 0, Op(Not(MetaVar 0))))) in
